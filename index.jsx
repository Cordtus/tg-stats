...
const ChatDashboard = () => {
  ...
  useEffect(() => {
    const analyzeFiles = async () => {
      ...
      try {
        ...
        const absoluteMinDate = new Date(
          Math.min(...allDates.map((date) => new Date(date)))
        );
        const absoluteMaxDate = new Date(
          Math.max(...allDates.map((date) => new Date(date)))
        );
        ...
        <div className="max-w-4xl mx-auto py-12 px-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-xl p-6">
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 flex flex-col items-center">
              ...
              <label
                htmlFor="file-upload"
                className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600 cursor-pointer transition-colors duration-200"
              >
                Select Files
              </label>
  ...
  const weeklyData = Object.entries(metrics.messagesByDayOfWeek).map(([day, count]) => ({
    day,
    messages: count,
  }));
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <CollapsibleSection title="Daily Activity">
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={filteredDailyData} className="dark:bg-gray-800">
            ...
            <CollapsibleSection title="Message Distribution by Hour">
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={hourlyData} className="dark:bg-gray-800">
                    <CartesianGrid stroke="#4a5568" strokeDasharray="3 3" />
                    <XAxis dataKey="hour" stroke="#718096" tick={{ fill: '#718096' }} />
                    <YAxis stroke="#718096" tick={{ fill: '#718096' }} />
                    <Tooltip contentStyle={{ backgroundColor: '#2d3748', color: '#e2e8f0' }} />
                    <Legend wrapperStyle={{ color: '#718096' }} />
                    <Bar dataKey="messages" fill="#82ca9d" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CollapsibleSection>
        ...
        {activeTab === 'activity' && (
          <div className="space-y-6">
            <CollapsibleSection title="Activity by Day of Week">
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData} className="dark:bg-gray-800">
                    <CartesianGrid stroke="#4a5568" strokeDasharray="3 3" />
                    <XAxis dataKey="day" stroke="#718096" tick={{ fill: '#718096' }} />
                    <YAxis stroke="#718096" tick={{ fill: '#718096' }} />
                    <Tooltip contentStyle={{ backgroundColor: '#2d3748', color: '#e2e8f0' }} />
                    <Legend wrapperStyle={{ color: '#718096' }} />
            ...
            {['Most Active Users', 'Most Reacted To', 'Most Mentioned Users'].map((title, index) => (
              <CollapsibleSection key={title} title={title}>
                <SortableList
                  items={
                    index === 0
                      ? metrics.mostActiveUsers.map(({ user, count }) => [user, count])
                      : index === 1
                      ? metrics.mostReactedTo.map(({ user, count }) => [user, count])
                      : metrics.topMentions
                  }
                  sortOptions={[
                    { key: 'name', label: 'Username' },
                    {
                      key: 'count',
                      label:
                        index === 0 ? 'Message Count' : index === 1 ? 'Reactions' : 'Mentions',
                    },
                  ]}
                  defaultSort="count"
